...
export function OverviewPage() {
  const { data: statsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return json.success ? json.data : { totalSessions: 0 };
    }
  });
  ...
  const stats = [
    { label: "Active Agents", value: statsData?.totalSessions?.toString() || "0", icon: Users, trend: "+1", color: "text-indigo-400" },
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="py-8 md:py-10 lg:py-12 space-y-8 animate-fade-in relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat) => (
            <Card key={stat.label} className="bg-zinc-900/40 border-zinc-800 backdrop-blur-sm group hover:border-zinc-700 hover:shadow-glow transition-all duration-300">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-zinc-800">
                {recentSessions?.length === 0 ? (
                  <div className="p-8 text-center text-zinc-600 text-xs italic">No recent session data available.</div>
                ) : (
                  recentSessions?.map((session) => (
                    <div key={session.id} className="flex items-center justify-between p-4 hover:bg-zinc-800/50 transition-colors">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className="px-1.5 py-0.5 rounded bg-indigo-500/10 text-[10px] font-mono text-indigo-400">AGENT</div>
              ...
              <div className="p-4 bg-zinc-900/20">
                <Button variant="link" className="w-full text-zinc-500 hover:text-emerald-400 text-[10px] uppercase font-bold tracking-widest gap-2">
                  View All Agents <ArrowUpRight className="h-3 w-3" />
                </Button>
              </div>
            </CardContent>
          </Card>
